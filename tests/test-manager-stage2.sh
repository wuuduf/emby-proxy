...
EOF
cat >"$TMP_DIR/state/sites.d/domain-test.example.com.json" <<EOF
{"schema_version":2,"id":"domain-test.example.com","engine":"caddy","domain":"test.example.com","listen_port":443,"entry_type":"subdomain","public_url":"https://test.example.com","managed_kind":"standalone","config_file":"$TMP_DIR/caddy/Caddyfile","created_at":"2026-08-17T00:00:00Z","updated_at":"2026-08-17T00:00:00Z","routes":[{"path":"/","upstream":"http://127.0.0.1:8096"}]}
EOF
cat >"$TMP_DIR/os-release" <<'EOF'
PRETTY_NAME="Test Debian"
VERSION="12 (bookworm)"
EOF
cat >"$TMP_DIR/log/emby-proxy-test.example.com-access.log" <<EOF
...
EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  ...
  show_stats domain-test.example.com 24h >"$EMBY_PROXY_STATE_HOME/stats.out"
  view_logs domain-test.example.com --media >"$EMBY_PROXY_STATE_HOME/media.out"
  view_logs domain-test.example.com --errors >"$EMBY_PROXY_STATE_HOME/errors.out"
' || fail "日志/统计功能运行失败"
...
# 域名入口诊断在服务、监听、配置、健康和源站都正常时不应产生错误。
EMBY_PROXY_STATE_HOME="$TMP_DIR/state" EMBY_PROXY_CADDYFILE="$TMP_DIR/caddy/Caddyfile" \
EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  set --
  source "$MANAGER_UNDER_TEST"
  ...
  curl() { printf 200; }
  doctor_dns() { doctor_ok "DNS 正常"; }
  doctor_certificate() { doctor_ok "证书正常"; }
  doctor_firewall() { doctor_ok "防火墙正常"; }
  doctor_site "$EMBY_PROXY_STATE_HOME/sites.d/domain-test.example.com.json" "" >"$EMBY_PROXY_STATE_HOME/doctor.out"
  [[ "$DOCTOR_ERROR" == 0 ]]
' || fail "入口诊断产生错误"
grep -F '本机入口健康检查 HTTP 200' "$TMP_DIR/state/doctor.out" >/dev/null || fail "诊断缺少健康检查"

# 只有 Caddy 入口时，Nginx 计数为 0 不能被 pipefail 当成错误。
EMBY_PROXY_STATE_HOME="$TMP_DIR/state" EMBY_PROXY_CADDYFILE="$TMP_DIR/caddy/Caddyfile" \
EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  set --
  source "$MANAGER_UNDER_TEST"
  ...
  ss() { printf "LISTEN 0 4096 *:443 *:* users:((\"caddy\",pid=123,fd=7))\n"; }
  print_status >"$EMBY_PROXY_STATE_HOME/status.out"
' || fail "零项引擎/模式统计导致状态页面退出"
grep -F '托管入口：1（域名 HTTPS）' "$TMP_DIR/state/status.out" >/dev/null || fail "状态页面入口计数错误"
grep -F '引擎分布：Caddy 1，Nginx 0' "$TMP_DIR/state/status.out" >/dev/null || fail "状态页面引擎计数错误"
grep -F '*:443' "$TMP_DIR/state/status.out" >/dev/null || fail "状态页面未显示实际托管监听端口"
...
EMBY_PROXY_OS_RELEASE_FILE="$TMP_DIR/os-release" EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  ...
  doctor_certificate() { doctor_ok "证书正常"; }
  doctor_firewall() { doctor_ok "防火墙正常"; }
  run_doctor domain-test.example.com >"$EMBY_PROXY_STATE_HOME/full-doctor.out"
' || fail "完整诊断读取 os-release 失败"
grep -F '系统：Test Debian' "$TMP_DIR/state/full-doctor.out" >/dev/null || fail "完整诊断未读取系统名称"

# 源站任何有效 HTTP 状态（包括 5xx）都代表固定代理链路可达。
EMBY_PROXY_STATE_HOME="$TMP_DIR/state" EMBY_PROXY_CADDYFILE="$TMP_DIR/caddy/Caddyfile" \
EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  set --
  source "$MANAGER_UNDER_TEST"
  ...
  doctor_certificate() { doctor_ok "证书正常"; }
  doctor_firewall() { doctor_ok "防火墙正常"; }
  doctor_site "$EMBY_PROXY_STATE_HOME/sites.d/domain-test.example.com.json" "" >"$EMBY_PROXY_STATE_HOME/doctor-403.out"
  [[ "$DOCTOR_ERROR" == 0 && "$DOCTOR_OK" -ge 1 ]]
' || fail "源站 5xx 被错误诊断为不可达"
grep -F '源站可访问：http://127.0.0.1:8096（HTTP 503）' "$TMP_DIR/state/doctor-403.out" >/dev/null || fail "源站 5xx 成功文案缺失"
...
EMBY_PROXY_NGINX_CONF_DIR="$TMP_DIR/nginx" EMBY_PROXY_MANAGER_LIB_ONLY=1 MANAGER_UNDER_TEST="$MANAGER" bash -c '
  ...
  dir="$(backup_target domain-test.example.com "$EMBY_PROXY_CADDYFILE" test-backup)"
